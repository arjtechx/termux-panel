...
async function validateAll(mode) {
  const checks = [];
  if (mode !== 'cloudflare_only') checks.push({ key:'nginx', ...(await run('nginx -t')) });
  if (mode !== 'proxy_only') checks.push({ key:'cloudflared', ...(await run('cloudflared tunnel ingress validate')) });
